...
function get_all_tasks($conn, $page = 1, $per_page = 10)
{
...
function get_total_pages($conn, $per_page = 10)
{
	$sql = "SELECT COUNT(*) as total FROM tasks";
	$stmt = $conn->prepare($sql);
	$stmt->execute();
	$result = $stmt->fetch();
	return ceil($result['total'] / $per_page);
}
/* 
 ...
 */
function get_all_tasks_due_today($conn, $page = 1, $per_page = 10)
{
	$offset = ($page - 1) * $per_page;
	$sql = "SELECT * FROM tasks WHERE due_date = CURDATE() AND status != 'completed' ORDER BY id DESC LIMIT :limit OFFSET :offset";
	$stmt = $conn->prepare($sql);
	$stmt->bindValue(':limit', (int)$per_page, PDO::PARAM_INT);
	$stmt->bindValue(':offset', (int)$offset, PDO::PARAM_INT);
	$stmt->execute();
...
function get_all_tasks_overdue($conn, $page = 1, $per_page = 10)
{
	$offset = ($page - 1) * $per_page;
	$sql = "SELECT * FROM tasks WHERE due_date < CURDATE() AND status != 'completed' ORDER BY id DESC LIMIT :limit OFFSET :offset";
	$stmt = $conn->prepare($sql);
...
function get_all_tasks_NoDeadline($conn, $page = 1, $per_page = 10)
{
	$offset = ($page - 1) * $per_page;
	$sql = "SELECT * FROM tasks WHERE status != 'completed' AND due_date IS NULL OR due_date = '0000-00-00' ORDER BY id DESC LIMIT :limit OFFSET :offset";
	$stmt = $conn->prepare($sql);
	$stmt->bindValue(':limit', (int)$per_page, PDO::PARAM_INT);
	$stmt->bindValue(':offset', (int)$offset, PDO::PARAM_INT);
	$stmt->execute();
